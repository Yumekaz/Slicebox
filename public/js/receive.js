...
(function() {
  ...
  if (!hash.startsWith('#box=')) {
    document.getElementById('receive-screen').innerHTML =
      '<div style="text-align:center;padding:80px 20px"><p style="font-size:13px;color:var(--muted);letter-spacing:2px">No box found. <a href="index.html">Build one →</a></p></div>';
    return;
  }

  const data = SB.decodeBox(hash.slice(5));
  if (!data || !data.selection || !data.card) {
    document.getElementById('receive-screen').innerHTML =
      '<div style="text-align:center;padding:80px 20px"><p style="font-size:13px;color:var(--muted);letter-spacing:2px">Invalid link. <a href="index.html">Build one →</a></p></div>';
    return;
  }

  const { selection, card } = data;
  const n = Math.min(selection.length, 6);

  // From line
  document.getElementById('from-line').textContent = `from ${card.from}`;
  ...
  const count = Math.min(Math.max(selection.length, 6), 10);
  ...
  for(let i=0; i<count; i++) {
    placed.push({ id: selection[i], pos: layout[i] });
  }
  ...
  // Reveal card after short delay (no box to open anymore)
  setTimeout(() => {
    const cardEl = document.getElementById('gift-tag');
    if (cardEl) { cardEl.style.opacity = '1'; }
  }, 400);

  // Fill card
  document.getElementById('tag-dear').textContent = `Dear ${card.to},`;
  document.getElementById('tag-msg').textContent   = card.msg;
  document.getElementById('tag-sign').textContent  = `Sincerely, ${card.from}`;
})();
